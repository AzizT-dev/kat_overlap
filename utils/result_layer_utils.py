...
class ResultLayerBuilder:
    ...
    def create_result_layer(dialog, results: List[Dict[str, Any]]) -> Optional[QgsVectorLayer]:
        ...
        def tr(msg: str) -> str:
            try:
                if dialog is not None and hasattr(dialog, "tr"):
                    return dialog.tr(msg)
            except Exception:
                pass
            return msg
        ...
        def _log(level: str, msg: str) -> None:
            try:
                if dialog is not None and hasattr(dialog, "_log_message"):
                    dialog._log_message(level, msg)
                else:
                    print(f"[{level}] {msg}")
            except Exception:
                print(f"[{level}] {msg}")
        ...
        try:
            ...
            if hasattr(dialog, "radio_crs_custom") and getattr(dialog, "radio_crs_custom").isChecked():
                try:
                    crs = dialog.crs_selector.crs()
                except Exception:
                    pass

            layer_name = tr("Overlap_results_{}").format(len(results))
            ...
            for idx, r in enumerate(results):
                ...
                # geom_type attribute: set a provisional value (will be consistent with chosen_type)
                try:
                    type_name = QgsWkbTypes.displayString(geom.wkbType())
                except Exception:
                    type_name = "Unknown"
                feat.setAttribute(7, type_name.lower())

                try:
                    feat.setGeometry(geom)
                except Exception as e:
                    _log("warning", tr("#{} unable to assign geometry: {}").format(idx, e))
                    geom_empty_count += 1
                    continue
            ...
            def choose_geom_type(detected_types_set):
                try:
                    if not detected_types_set:
                        return "GeometryCollection"

                    s = {str(g).lower() for g in detected_types_set}
                    ...
                    _log("warning", tr("choose_geom_type error: {}, fallback GeometryCollection").format(e))
                    return "GeometryCollection"
            ...
            if features_to_add:
                try:
                    ...
                    _log("error", tr("Error adding features: {}").format(e))
                    try:
                ...
                if added_count > 0:
                    ...
                    try:
                        ...
                        _log("warning", tr("Unable to add result layer to project (continuing)"))

                    _log("info", tr("Layer created: {} ({} features)").format(layer_name, added_count))

                    # Enable export button if available
                    try:
                        if dialog is not None and hasattr(dialog, "export_layer_btn") and dialog.export_layer_btn is not None:
                    ...
                    _log("error", tr("No features could be added to the layer!"))
                    return None
            else:
                _log("warning", tr("No valid features to add: {} empty/invalid, total={}").format(geom_empty_count, len(results)))
                return None

        except Exception as e:
            _log("error", tr("Error creating results layer: {}").format(e))
            return None
