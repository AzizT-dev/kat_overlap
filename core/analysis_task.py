...
from .classification import PresetManager, tr
import math
...
class AnalysisTask(QgsTask):
    ...
    def __init__(self, description: str, layers: Dict[str, Any], params: Optional[Dict[str, Any]] = None,
        ...
        profile_name = self.params.get('profile', tr('Foncier/Cadastre (GPS ±2m)'))
            ...
            if callable(self.on_finished):
                try:
                    ...
                    if callable(self.on_error):
                        self.on_error(tr("Callback on_finished failed: {}").format(e))
        else:
            msg = self._error_msg or tr("Analysis failed.")
            if callable(self.on_error):
                self.on_error(msg)

    def cancelled(self) -> None:
        self._log(tr("Analysis cancelled by user."))
        if callable(self.on_error):
            self.on_error(tr("Analysis cancelled."))
    ...
    def _get_real_id(self, feature, layer):
        try:
            layer_id = layer.id()
            if layer_id in self.id_fields and self.id_fields[layer_id] != tr("Aucun"):
    ...
    def _analyze_self_overlaps(self, layer) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self._log(tr("Polygon layer read error: {}").format(e))
            return res
        ...
        for i, feat_a in enumerate(feats_snap):
            ...
            for feat_b in feats_snap[i+1:]:
                ...
                try:
                    ...
                    self._log(tr("asJson() self_overlap error: {}").format(e))
            ...
            try:
                ...
                pass
            self._log_progress(tr("Polygon analysis {}/{}").format(i+1, total))
    ...
    def _analyze_point_proximity(self, layer) -> List[Dict[str, Any]]:
        ...
        for lid, field_name in self.id_fields.items():
            if layer and layer.id() == lid:
                id_field = field_name if field_name != self.tr("Aucun") else None
                break
    ...
    def _analyze_points_in_polygons(self, point_layer, poly_layer) -> List[Dict[str, Any]]:
        ...
        for lid, field_name in self.id_fields.items():
            if point_layer and point_layer.id() == lid:
                id_field_points = field_name if field_name != self.tr("Aucun") else None
            if poly_layer and poly_layer.id() == lid:
                id_field_polys = field_name if field_name != self.tr("Aucun") else None
